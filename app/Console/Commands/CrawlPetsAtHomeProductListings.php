...
use App\Crawler\Scrapers\PetsAtHomeCrawler;
use App\Jobs\Crawler\CrawlProductListingsJob;
use Illuminate\Console\Command;

class CrawlPetsAtHomeProductListings extends Command
{
    protected $signature = 'crawler:petsathome
                            {--queue= : The queue to dispatch jobs to}
                            {--sync : Run synchronously instead of queuing}';

    protected $description = 'Crawl Pets at Home product listings for dog food and treats';

    public function handle(): int
    {
        $this->info('Starting Pets at Home product listing crawler...');

        $crawler = new PetsAtHomeCrawler(new GuzzleHttpAdapter);
        ...
        foreach ($startingUrls as $url) {
            ...
            $job = new CrawlProductListingsJob(
                crawlerClass: PetsAtHomeCrawler::class,
                url: $url,
                crawlId: null,
