...
use App\Crawler\Scrapers\SainsburysCrawler;
use App\Jobs\Crawler\CrawlProductListingsJob;
use Illuminate\Console\Command;

class CrawlSainsburysCommand extends Command
{
    protected $signature = 'crawler:sainsburys
                            {--queue= : The queue to dispatch jobs to}
                            {--sync : Run synchronously instead of queuing}';

    protected $description = "Crawl Sainsbury's product listings for dog food and treats";

    public function handle(): int
    {
        $this->info("Starting Sainsbury's product listing crawler...");

        $crawler = new SainsburysCrawler(new GuzzleHttpAdapter);
        ...
        foreach ($startingUrls as $url) {
            ...
            $job = new CrawlProductListingsJob(
                crawlerClass: SainsburysCrawler::class,
                url: $url,
                crawlId: null,
            );
