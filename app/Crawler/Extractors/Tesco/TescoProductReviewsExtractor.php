...
class TescoProductReviewsExtractor implements ExtractorInterface
{
    /**
     * Extract reviews from Tesco product pages.
     *
     * Tesco product pages may contain reviews in various formats:
     * - Embedded in JSON-LD structured data
     * - In a reviews section on the page
     * - Via API endpoints (handled separately)
     */
    public function extract(string $html, string $url): Generator
    {
        ...
        Log::info("TescoProductReviewsExtractor: Extracted {$reviewsExtracted} reviews from {$url}");
    }
    ...
    private function extractFromJsonLd(Crawler $crawler, string $url): Generator
    {
        ...
        try {
            ...
            Log::debug("TescoProductReviewsExtractor: JSON-LD extraction failed: {$e->getMessage()}");
        }
    }
    ...
    private function extractFromDom(Crawler $crawler, string $url): Generator
    {
        ...
        foreach ($reviewSelectors as $selector) {
            try {
                ...
                Log::debug("TescoProductReviewsExtractor: DOM selector {$selector} failed: {$e->getMessage()}");
            }
        }
    }
    ...
    private function extractRatingFromDom(Crawler $node): ?float
    {
        ...
        $ratingSelectors = ['[data-rating]', '[data-score]', '[data-stars]', '.review-rating', '.rating', '[data-auto="rating"]'];
        ...
        try {
            $filledStars = $node->filter('.star-filled, .star-full, .fa-star:not(.fa-star-o), .icon-star-filled, .beans-star--filled');
    ...
    private function generateReviewId(string $url, ?string $author, string $body, int $index): string
    {
        ...
        return "tesco-review-{$hash}-{$index}";
    }
}
