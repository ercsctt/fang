...
class SainsburysProductReviewsExtractor implements ExtractorInterface
{
    /**
     * Extract reviews from Sainsbury's product pages.
     *
     * Reviews may be found in:
     * - JSON-LD structured data
     * - DOM review sections
     * - Bazaarvoice widget (common for Sainsbury's)
     */
    public function extract(string $html, string $url): Generator
    {
        ...
        Log::info("SainsburysProductReviewsExtractor: Extracted {$reviewsExtracted} reviews from {$url}");
    }
    ...
    private function extractFromJsonLd(Crawler $crawler, string $url): Generator
    {
        ...
        try {
            ...
            Log::debug("SainsburysProductReviewsExtractor: JSON-LD extraction failed: {$e->getMessage()}");
        }
    }
    ...
    {
        // Sainsbury's-specific review selectors
        $reviewSelectors = [
        ...
        foreach ($reviewSelectors as $selector) {
            try {
                ...
                Log::debug("SainsburysProductReviewsExtractor: DOM selector {$selector} failed: {$e->getMessage()}");
            }
        }
    }
    ...
    private function extractRatingFromDom(Crawler $node): ?float
    {
        ...
        // Try Sainsbury's specific rating selectors
        $ratingSelectors = [
    ...
    private function generateReviewId(string $url, ?string $author, string $body, int $index): string
    {
        ...
        return "sainsburys-review-{$hash}-{$index}";
    }
