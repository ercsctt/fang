...
class BMProductDetailsExtractor implements ExtractorInterface
{
    ...
    private const WEIGHT_TO_GRAMS = [
        'kg' => 1000,
        'g' => 1,
        'ml' => 1, // Treating ml as grams for simplicity (water density)
        'l' => 1000,
        'ltr' => 1000,
        'litre' => 1000,
        'litres' => 1000,
    ...
    {
        // Match patterns like "2.5kg", "400g", "500ml", "1l", "1.5 kg"
    ...
    private function extractCategory(Crawler $crawler, string $url): ?string
    {
        ...
        foreach ($breadcrumbSelectors as $selector) {
            try {
                $elements = $crawler->filter($selector);
                if ($elements->count() > 1) {
                    // Get all breadcrumb texts, skip first (usually Home) and last (product name)
                    $crumbs = $elements->each(fn (Crawler $node) => trim($node->text()));
                    $crumbs = array_filter($crumbs);
                    $crumbs = array_values($crumbs);

                    // Return the deepest category (second to last, before product)
                    if (count($crumbs) >= 2) {
                        // Skip "Home" and the product name itself
                        $categoryIndex = max(0, count($crumbs) - 2);
                        if (! in_array(strtolower($crumbs[$categoryIndex]), ['home', ''])) {
                            return $crumbs[$categoryIndex];
                        }
                    }
                }
            } catch (\Exception $e) {
                Log::debug("BMProductDetailsExtractor: Category breadcrumb selector {$selector} failed: {$e->getMessage()}");
            }
        }

        // Try to extract from URL
        if (preg_match('/\/(pet|dog|cat|puppy|kitten|treats|food)[s]?(?:\/|$)/i', $url, $matches)) {
