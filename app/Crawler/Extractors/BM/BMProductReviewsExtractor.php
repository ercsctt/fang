...
class BMProductReviewsExtractor implements ExtractorInterface
{
    /**
     * Extract reviews from B&M product pages.
     *
     * B&M product pages may contain reviews in various formats:
    ...
    public function extract(string $html, string $url): Generator
    {
        ...
        Log::info("BMProductReviewsExtractor: Extracted {$reviewsExtracted} reviews from {$url}");
    }
    ...
    private function extractFromJsonLd(Crawler $crawler, string $url): Generator
    {
        try {
            ...
            Log::debug("BMProductReviewsExtractor: JSON-LD extraction failed: {$e->getMessage()}");
        }
    }
    ...
    private function extractFromDom(Crawler $crawler, string $url): Generator
    {
        ...
        foreach ($reviewSelectors as $selector) {
            try {
                ...
                Log::debug("BMProductReviewsExtractor: DOM selector {$selector} failed: {$e->getMessage()}");
            }
        }
    }
    ...
    private function extractRatingFromDom(Crawler $node): ?float
    {
        ...
        $ratingSelectors = ['[data-rating]', '[data-score]', '[data-stars]', '.review-rating', '.rating'];
