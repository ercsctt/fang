...
class OcadoProductDetailsExtractor implements ExtractorInterface
{
    ...
    private function extractJsonLd(Crawler $crawler): array
    {
        try {
            ...
            Log::debug("OcadoProductDetailsExtractor: Failed to extract JSON-LD: {$e->getMessage()}");
        }
    ...
    private function extractCategory(Crawler $crawler, string $url): ?string
    {
        ...
        foreach ($breadcrumbSelectors as $selector) {
            try {
                ...
                Log::debug("OcadoProductDetailsExtractor: Category breadcrumb selector {$selector} failed: {$e->getMessage()}");
            }
        }
