...
class ZooplusProductReviewsExtractor implements ExtractorInterface
{
    /**
     * Extract reviews from Zooplus product pages.
     *
    ...
    public function extract(string $html, string $url): Generator
    {
        ...
        Log::info("ZooplusProductReviewsExtractor: Extracted {$reviewsExtracted} reviews from {$url}");
    }
    ...
    private function extractFromJsonLd(Crawler $crawler, string $url): Generator
    {
        ...
        try {
            ...
            Log::debug("ZooplusProductReviewsExtractor: JSON-LD extraction failed: {$e->getMessage()}");
        }
    }
    ...
    {
        // Zooplus-specific review selectors
        $reviewSelectors = [
            '[data-zta="reviewItem"]',
        ...
        foreach ($reviewSelectors as $selector) {
            try {
                ...
                Log::debug("ZooplusProductReviewsExtractor: DOM selector {$selector} failed: {$e->getMessage()}");
            }
        }
    }
    ...
    private function extractRatingFromDom(Crawler $node): ?float
    {
        ...
        // Try Zooplus specific rating selectors
        $ratingSelectors = [
        ...
        try {
            $filledStars = $node->filter('.star-filled, .star-full, .fa-star:not(.fa-star-o), .icon-star-filled, .star.active');
        ...
        try {
            $ratingNode = $node->filter('.rating-stars, .star-rating');
            if ($ratingNode->count() > 0) {
    ...
    private function generateReviewId(string $url, ?string $author, string $body, int $index): string
    {
        ...
        return "zooplus-review-{$hash}-{$index}";
    }
