...
class JFPProductReviewsExtractor implements ExtractorInterface
{
    /**
     * Extract reviews from Just for Pets product pages.
     *
    ...
    public function extract(string $html, string $url): Generator
    {
        ...
        Log::info("JFPProductReviewsExtractor: Extracted {$reviewsExtracted} reviews from {$url}");
    }
    ...
    private function extractFromJsonLd(Crawler $crawler, string $url): Generator
    {
        ...
        try {
            ...
            Log::debug("JFPProductReviewsExtractor: JSON-LD extraction failed: {$e->getMessage()}");
        }
    }
    ...
    private function extractFromDom(Crawler $crawler, string $url): Generator
    {
        ...
        foreach ($reviewSelectors as $selector) {
            try {
                ...
                Log::debug("JFPProductReviewsExtractor: DOM selector {$selector} failed: {$e->getMessage()}");
            }
        }
    }
    ...
    private function extractRatingFromDom(Crawler $node): ?float
    {
        ...
        $ratingSelectors = ['[data-rating]', '[data-score]', '[data-stars]', '.review-rating', '.rating', '.star-rating'];
